...
def example_gnn_data(example_data):
    ...
    polymer_smiles = example_data["data_df"].loc[polymer_indices, "smiles"].values.tolist()
    ...
    dataset = [Data(x=tensor(featurize_smiles_by_atom(polymer_smiles[ind], feature_dict, max_n_atoms, n_features), dtype=torch_float),
    ...
    return {
        "GraphNet1_class_ls": [
            lambda : GraphNet1(projector_dim, 1, capacity, n_features, max_n_atoms) for capacity in example_data["capacity_ls"]
